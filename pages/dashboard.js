...
function CRUDPengumuman(){
  ...
  return (<div className="card" style={{marginTop:16}}>
    ...
    <ul>{items.map(it=>(<li key={it.id}><b>{it.judul}</b> — {it.isi||""}</li>))}</ul>
  </div>);
}
...
function CRUDAgenda(){
  ...
  return (<div className="card" style={{marginTop:16}}>
    ...
    <ul>{items.map(it=>(<li key={it.id}><b>{it.nama}</b> — {it.waktu} @ {it.lokasi}</li>))}</ul>
  </div>);
}
...
function CRUDLaporan(){
  ...
  return (<div className="card" style={{marginTop:16}}>
    ...
    <ul>{items.map(it=>(<li key={it.id}><b>{it.nama}</b> — {it.deskripsi} [{it.status||"pending"}] 
...
function CRUDIuran(){
  ...
  return (<div className="card" style={{marginTop:16}}>
    ...
    <ul>{items.map(it=>(<li key={it.id}>
      [{it.kategori?.toUpperCase()}] {it.bulan} — Rp{it.jumlah} — {it.metode} — <b>{it.status}</b>
      {it.buktiUrl && <> — <a href={it.buktiUrl} target="_blank" rel="noreferrer">Bukti</a></>}
...
function CRUDUMKM(){
  ...
  return (<div className="card" style={{marginTop:16}}>
    ...
    <textarea className="input" placeholder="Deskripsi" value={deskripsi} onChange={e=>setDeskripsi(e.target.value)} />
    <button className="btn" onClick={add} style={{marginTop:8}}>Tambah</button>
    <ul>{items.map(it=>(<li key={it.id}><b>{it.nama}</b> — {it.deskripsi} ({it.wa})
